...
namespace FormuleParser {
  ...
  int skipSpaces(string input, int from) {
    bool notfound = true;
    while (from < input.length() && notfound) {
    ...
    if (lvl <= 0) { // parens or variable
      ...
      if (is_binary(op)) {
        ...
        if ( input.compare(pr1.idx, s_op.length(), s_op) == 0 ) {
          parse_result pr2 = read_lvl(input,pr1.idx+s_op.length(), lvl);
          parse_result res;
          res.f = new formule();
          res.f -> op = op;
          res.f -> arg1 = pr1.f;
        ...
        if ( input.compare(from, s_op.length(), s_op) == 0 )  {
          parse_result pr = read_lvl(input,from + s_op.length(),lvl);
          parse_result res;
          res.idx = pr.idx;
          res.f = new formule();
          res.f->op = op;
